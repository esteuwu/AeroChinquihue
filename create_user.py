...
if len(sys.argv) < 4:
    ...
    sys.exit(3)
if not mvvm.view.Identification.is_identification_valid(sys.argv[2]):
...
cursor.execute("INSERT INTO users VALUES (?, ?, ?, ?);", (mvvm.view.Identification(sys.argv[2]).get_raw_identification(), sys.argv[1], base64.b64encode(hashed_password).decode(), base64.b64encode(salt).decode()))
connection.commit()
